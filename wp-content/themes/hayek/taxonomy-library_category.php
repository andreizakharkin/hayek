...
$mytermidord = $myterm->term_id;
				if($order_cat=='all') { 
					global $wpdb;
					
					$queried_object = get_queried_object();
					$mytermid = $queried_object->term_id;
					$mbsqlstr ="select ID from $wpdb->posts p, $wpdb->term_taxonomy tt, $wpdb->terms t, $wpdb->term_relationships tr
						...
						and t.term_id=$mytermid
						union
						select ID from $wpdb->posts p
						where p.post_type = 'post' AND p.post_status = 'publish'
						and p.id in( select post_id from $wpdb->postmeta where  meta_key = 'show' AND meta_value in('1') ) 
						and p.id in( select post_id from $wpdb->postmeta where  meta_key = 'lcat2' AND meta_value in('$mytermid') ) 
						";						
					...
					$mbsqlstr ="select ID from $wpdb->posts p, $wpdb->term_taxonomy tt,  $wpdb->terms t,  $wpdb->term_relationships tr
						...
						where p.post_type = '".APP_LIBRARY_TYPE."' AND p.post_status = 'publish' and p.id= tr.object_id and tr.term_taxonomy_id =tt.term_taxonomy_id
						 AND tt.term_id = t.term_id and t.term_id=$mytermidord 
						 
						 and p.ID in(select ID from $wpdb->posts p, $wpdb->term_taxonomy tt,  $wpdb->terms t,  $wpdb->term_relationships tr
						where p.post_type = '".APP_LIBRARY_TYPE."' AND p.post_status = 'publish' and p.id= tr.object_id and tr.term_taxonomy_id =tt.term_taxonomy_id
						 AND tt.term_id = t.term_id and t.term_id=$mytermid) 
	 union
						select ID from $wpdb->posts p
						where p.post_type = 'post' AND p.post_status = 'publish'
						and p.id in( select post_id from $wpdb->postmeta where  meta_key = 'show' AND meta_value in('1') ) 
						and (p.id in( select post_id from $wpdb->postmeta where  meta_key = 'lcat' AND meta_value in('$mytermidord') ) 
						and p.id in( select post_id from $wpdb->postmeta where  meta_key = 'lcat2' AND meta_value in('$mytermid') ))
						"; 
...
//				 echo $mbsqlstr;
		...
		<div class="item">
			...
			<div class="date"><?php echo get_the_date('d.m.Y | G:i'); ?></div>
			<p><?php truncate_post(150); ?></p>
			<p class="more"><a class="otherLink" href="<?php the_permalink(); ?>"><span>read more</span> &gt;</a></p>
                        <div class="clearfix"></div>
                </div>			
			<?php			
					break;
		    ...
		    <input type="hidden" value="2" id="morepgd" name="morepgd">
                ...
                <div class="navPanel mobileNone tabletNone">
                    <?php /* wp_corenavi($query_string.'&posts_per_page='.$posts_per_page.$order_cat,true); */ wp_corenavi($query_array,true);  ?>
                    <div class="clearfix"></div>
                </div>
            </div>
